...
export default function CardMsds({ color="light" }) {
  ...
  const data = useMemo(() => {
    ...
    const msdsFiltered = msds.map( ({ id, capacidad }) => ({
      col1: `${ capacidad } GB`,
      col2: <CardEditDeleteDropdown
              path={`/admin/msd-new/${ id }?last_url=${ location.pathname }`}
              id={ id }
              table="msd"
