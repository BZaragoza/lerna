...
export default function CardSims({ color="light" }) {
  ...
  const data = useMemo(() => {
    ...
    const simsFiltered = sims.map(({ id, compañia }) => ({
      col1: compañia,
      col2: <CardEditDeleteDropdown
        path={`/admin/sim-new/${id}?last_url=${location.pathname}`}
        id={id}
        table="chip"
        ...
        <div className="flex flex-wrap items-center">
          <div className="relative w-full px-4 max-w-full flex-grow flex-1">
            <div className="flex justify-between">
              ...
              <Link to={`/admin/sim-new?last_url=${location.pathname}`}>
                <button
